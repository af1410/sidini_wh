                            ...
                            <tbody>
                                @foreach ($nilai as $item)
                                    <tr>
                                        ...
                                        @else
                                            <td>{{ number_format($item->nilai_tes_tulis, 0, 0) }}</td>
                                            <td>{{ number_format($item->nilai_kehadiran, 0, 0) }}</td>
                                            <td>{{ number_format($item->tugas->avg('nilai') ?? 0, 0) }}</td>
                                            <td>{{ number_format($item->nilai_bab, 0, 0) }}</td>
                                        @endif
                                        <td>
                        ...
                        <div class="mt-4">
                            ...
                            @foreach ($nilai as $item)
                                @if ($item->tugas->isNotEmpty())
                                    <div class="mb-3">
                                        <strong>{{ $item->siswa->nama_siswa ?? 'Siswa' }}</strong>
                                        <ul class="list-group mt-2">
                                            @foreach ($item->tugas as $tugas)
                                                <li
                                                    class="list-group-item d-flex justify-content-between align-items-center">
                                                    {{ $tugas->nama_tugas }}
                                                    <span class="badge bg-primary">{{ $tugas->nilai }}</span>
                                                </li>
                                            @endforeach
                                        </ul>
                                    </div>
