...
@section('content')
    ...
    <div class="container-fluid">
                ...
                <form action="{{ route('admin.guru.index') }}" method="GET">
                    <div class="row g-3">
                        ...
                        <div class="col-md-2 d-flex align-items-end gap-2">
                            ...
                            <a href="{{ route('admin.guru.index') }}" class="btn btn-secondary w-100">
                                Reset
                            </a>
                        </div>
                    </div>
        ...
        <div class="card mb-3 d-none" id="bulkActionCard">
            <div class="card-body">
                <form action="{{ route('admin.guru.bulk-status') }}" method="POST" id="bulkForm">
                    @csrf
                    @method('PUT')
                    ...
                    <tbody>
                        @forelse($data as $guru)
                            <tr>
                                <td>
                                    <div class="form-check m-0">
                                        <input class="form-check-input row-check" type="checkbox" value="{{ $guru->id_guru }}">
                                    </div>
                                </td>
                                ...
                                <td>{{ $guru->username }}</td>
                                <td>
                                    <a href="{{ route('admin.guru.edit', $guru->id_guru) }}" class="btn btn-sm btn-primary">
                                        <i class="bi bi-pencil me-1"></i>Edit
                                    </a>
                                    <button type="button" class="btn btn-sm btn-warning" data-bs-toggle="modal" data-bs-target="#statusModal{{ $guru->id_guru }}">
                                        <i class="bi bi-arrow-repeat me-1"></i>Status
                                    </button>
                                </td>
                            </tr>
                            <div class="modal fade" id="statusModal{{ $guru->id_guru }}" tabindex="-1" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <form action="{{ route('admin.guru.update-status', $guru->id_guru) }}" method="POST">
                                            @csrf
                                            @method('PUT')
                                            ...
                                            <div class="modal-body">
                                                ...
                                                <div class="mb-3">
                                                    <label class="form-label">Jabatan</label>
                                                    <input type="text" class="form-control" value="{{ ucwords(str_replace('_', ' ', $guru->jabatan)) }}" readonly>
                                                </div>
                                                <div class="mb-3">
                                                    <label class="form-label">Status Saat Ini</label>
                                                    <input type="text" class="form-control" value="{{ ucfirst($guru->status) }}" readonly>
                                                </div>
                                                <div class="mb-3">
